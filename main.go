...
const dbConnString   = "root@/importgo"
...
func main()  {
	...
	readCsvFilePaperLineThenSendToWorker(csvReader, jobs, wg)
...
func readCsvFilePaperLineThenSendToWorker(csvReader *csv.Reader, jobs chan <- []interface{}, wg *sync.WaitGroup) {
	for {
...
func doTheJob(workerIndex, counter int, db *sql.DB, values []interface{}) {
	...
	if counter%1000 == 0 {
		log.Println("=> worker", workerIndex, "inserted", counter, "date")
	}
}
